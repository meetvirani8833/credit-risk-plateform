      ...
      {open && (
        <div className="flex flex-col gap-4 border-t border-(--color-border) px-5 py-4">
          <p className="text-sm leading-relaxed text-(--color-ink-muted)">
            The chatbot can only answer from 3 tables. A question outside this scope (e.g. asking
            for a phone number, or anything from the other 4 raw dataset files not loaded here)
            gets an honest "can't answer that" instead of a guess.
          </p>
          {TABLES.map((t) => (
            <div key={t.name}>
